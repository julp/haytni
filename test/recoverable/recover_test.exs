defmodule Haytni.Recoverable.RecoverTest do
  ...
  describe "Haytni.RecoverablePlugin.recover/3" do
    ...
    test "ensures error when reset token has expired (and password remains the same)", %{config: config, user: user = %User{id: id, encrypted_password: encrypted_password}} do
      ...
      assert {:error, changeset} = Haytni.RecoverablePlugin.recover(HaytniTestWeb.Haytni, config, new_password_change(user.reset_password_token, "unused new password"))
      refute is_nil(changeset.action)
      assert %{reset_password_token: [Haytni.RecoverablePlugin.expired_token_message()]} == Haytni.DataCase.errors_on(changeset)
    ...
    test "ensures password was reseted in normal condition", %{config: config, user: user} do
      ...
      assert {:ok, updated_user} = Haytni.RecoverablePlugin.recover(HaytniTestWeb.Haytni, config, new_password_change(user.reset_password_token, new_password))
    ...
    test "ensures new password respects minimal length", %{config: config, user: user} do
      assert {:error, changeset} = Haytni.RecoverablePlugin.recover(HaytniTestWeb.Haytni, config, new_password_change(user.reset_password_token, "1"))
