defmodule Haytni.Authenticable.ValidateCreateRegistrationTest do
  ...
  @password "1234"
  describe "Haytni.AuthenticablePlugin.validate_create_registration/1" do
    test "ensures password is hashed (bcrypt) on registration (encrypted_password field populated)" do
      changeset = %User{}
      |> Ecto.Changeset.change(password: @password)
      |> Haytni.AuthenticablePlugin.validate_create_registration()

      assert changeset.valid?
      assert String.starts_with?(Ecto.Changeset.get_change(changeset, :encrypted_password), "$2b$")
      assert Haytni.AuthenticablePlugin.check_password(Ecto.Changeset.apply_changes(changeset), @password)
    end
  end
end
