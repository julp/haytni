defmodule Haytni.Authenticable.SessionCreateControllerTest do
  ...
  describe "HaytniWeb.Authenticable.SessionController#create" do
    test "checks error on invalid credentials", %{conn: conn} do
      new_conn = post(conn, Routes.haytni_user_session_path(conn, :create), session_params_without_rememberme())
    ...
    test "checks successful authentication (scope: user)", %{conn: conn} do
      user = user_fixture()
      new_conn = post(conn, Routes.haytni_user_session_path(conn, :create), session_params_without_rememberme(user))
    ...
    test "checks successful authentication (scope: admin)", %{conn: conn} do
      ...
      |> post(Routes.admin_haytni_admin_session_path(conn, :create), session_params_without_rememberme(admin))
